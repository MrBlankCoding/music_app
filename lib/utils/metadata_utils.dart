class MetadataUtils {
  ...
  static ({String mainTitle, List<String> featuring}) extractFeaturing(
    String title,
  ) {
  ...
  static String cleanChannelName(String channel) {
    ...
    c = c.replaceAll(
      RegExp(r'\s*-?\s*(VEVO|Topic|Official|Music)$', caseSensitive: false),
      '',
    );
  ...
  parseYouTubeTitle(String rawTitle) {
    ...
    if (extracted.artist != null && extracted.title != null) {
      ...
      return (
        ...
        cleanedFullTitle:
            '${cleanTitle(extracted.artist!)} - ${titleWithFeat.mainTitle}',
      );
    } else {
  ...
  static bool isValidMetadata({String? artist, String? title}) {
    ...
    final garbage = RegExp(
      r'^(unknown|untitled|n/?a|null|undefined)$',
      caseSensitive: false,
    );
